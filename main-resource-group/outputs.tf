...
output "action_group_id" {
  ...
  value       = var.enable_budget ? azurerm_monitor_action_group.budget_alerts[0].id : null
}

output "action_group_name" {
  description = "Name of the budget action group (if enabled)"
  value       = var.enable_budget ? azurerm_monitor_action_group.budget_alerts[0].name : null
}
